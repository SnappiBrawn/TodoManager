...
<?php
    ...
    if($todo_trash_id == ''){
        ...
        if($check_todo_id_count > 0){
            $todo_trash_query ="UPDATE `todos` SET `todo_trash`='0' WHERE `todo_id`='$todo_trash_id'";
            $todo_trash_execute = mysqli_query($con, $todo_trash_query);
            if($todo_trash_execute){
                ?>
                    <script>alert('This todo restore successfully.'); location.href='../../todo.php';</script>
                <?php
            }
            else{
                ?>
                    <script>alert('sorry, this todo is not restored.'); location.href='../../todo.php';</script>
                <?php
            }
        }
    }
